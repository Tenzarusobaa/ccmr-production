// EditCounseling.js for GCO with student ID autocomplete and UNCONFIRMED option
import React, { useEffect, useState } from "react";
import "./AddCounseling.css"; // Reuse the same CSS

const EditCounseling = ({ isOpen, onClose, record, onRecordUpdated }) => {
  ...
  useEffect(() => {
    ...
    if (isOpen && record) {
      ...
      setFormData({
        ...
        status: record.status || "TO SCHEDULE",
        date: record.date || "",
        time: record.time || "",
        concern: record.concern || "",
        remarks: record.remarks || "",
        psychologicalCondition: record.psychologicalCondition || "UNCONFIRMED" // Changed default
      });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        alert("Error updating counseling record: " + result.message);
      }
    } catch (error) {
      console.error("Error:", error);
      alert("An error occurred while updating the counseling record.");
    } finally {
