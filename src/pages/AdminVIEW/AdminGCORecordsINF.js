// src/pages/AdminVIEW/AdminGCORecordsINF.js
...
const AdminGCORecordsINF = ({ userData, onLogout, onNavItemClick, onExitViewAs }) => {
  ...
  // Force viewType to be INF for this admin view (INF viewing psychological records)
  ...
  // Fetch infirmary psychological records (special endpoint for INF)
  const fetchAllRecords = async () => {
    try {
      ...
      const endpoint = `${API_BASE_URL}api/infirmary/counseling-records`;
      ...
      if (data.success) {
        ...
        throw new Error(data.error || 'Failed to fetch psychological records');
      }
    } catch (err) {
  ...
  // Fetch students with psychological records
  const fetchStudents = async () => {
    try {
      ...
      const endpoint = `${API_BASE_URL}api/student-counseling-records/psychological`;
    ...
    return (
      <div className={`office-records-container ${getOfficeClass()}`}>
        ...
        <div className="content">
          <div className="loading-state">Loading psychological records...</div>
        </div>
      </div>
    );
  }
        ...
        {displayData.data.length === 0 ? (
          <div className="empty-state">
            {isSearchMode && searchQuery
              ? "No psychological records found matching your search."
              : "No psychological records found."}
          </div>
        ) : (
          <DataTable
...
export default AdminGCORecordsINF;